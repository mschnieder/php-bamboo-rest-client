...
/**
 * Interact bamboo server with REST API.
 */
class BambooClient
{
	/**
	 ...
	 * @var string
	 */
	private $api_uri = '/rest/api/latest';
	...
	public function __construct(ConfigurationInterface $configuration = null, Logger $logger = null, $path = './')
	{
		...
		$this->json_mapper->undefinedPropertyHandler = [new \BambooRestApi\JsonMapperHelper(), 'setUndefinedProperty'];

		// create logger
		if ($logger) {
			$this->log = $logger;
		} else {
			$this->log = new Logger('BambooClient');
			$this->log->pushHandler(new StreamHandler(
				$configuration->getBambooLogFile(),
				$this->convertLogLevel($configuration->getBambooLogLevel())
			));
		}
	...
	public function exec($context, $post_data = null, $custom_request = null)
	{
		...
		if (!$response) {
			...
			if ($this->http_response != 200 && $this->http_response != 201) {
				throw new BambooException('CURL HTTP Request Failed: Status Code : '
	...
	protected function closeCURLHandle(array $chArr, $mh, $body, $result_code)
	{
		...
		if ($result_code != 200) {
			...
			throw new BambooException('CURL Error: = '.$body, $result_code);
		}
	}
	...
	{
		$host = $this->getConfiguration()->getBambooHost();
	...
	{
		$username = $this->getConfiguration()->getBambooUser();
		$password = $this->getConfiguration()->getBambooPassword();
	...
	/**
	 * Set a custom Bamboo API URI for the request.
	 *
